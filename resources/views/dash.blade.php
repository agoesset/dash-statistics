...
<body>
                            ...
                            <div class="card-body px-4 py-4-5">
                                <div class="row">
                                    ...
                                    <div class="col-md-8 col-lg-12 col-xl-12 col-xxl-7">
                                        ...
                                        <h6 class="font-extrabold mb-0" id=""></h6>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        ...
        // Hitung total gender
        const totalGender = Number(likeData.like_male) + Number(likeData.like_female);

        // Hitung persentase gender dengan mengalikan hasil pembagian dengan 100
        const percentMale = totalGender > 0 ? Math.round((Number(likeData.like_male) / totalGender) * 100) : 0;
        const percentFemale = totalGender > 0 ? Math.round((Number(likeData.like_female) / totalGender) * 100) : 0;

        // Hitung persentase generasi
        const totalGeneration = Number(likeData.like_gen_alpha) + Number(likeData.like_gen_z) + Number(likeData.like_gen_millenial) + Number(likeData.like_baby_boomer);

        // Hitung persentase untuk setiap generasi dengan konversi ke angka
        const percentGenAlpha = totalGeneration > 0 ? Math.round((Number(likeData.like_gen_alpha) / totalGeneration) * 100) : 0;
        const percentGenZ = totalGeneration > 0 ? Math.round((Number(likeData.like_gen_z) / totalGeneration) * 100) : 0;
        const percentMillennial = totalGeneration > 0 ? Math.round((Number(likeData.like_gen_millenial) / totalGeneration) * 100) : 0;
        const percentBabyBoomer = totalGeneration > 0 ? Math.round((Number(likeData.like_baby_boomer) / totalGeneration) * 100) : 0;

        // Data untuk diagram gender (Male vs Female)
        const optionsVisitorsProfile = {
            series: [percentMale, percentFemale], // Persentase gender
            labels: ["Male", "Female"],
        ...
        const optionsGenerationalProfile = {
            series: [percentGenAlpha, percentGenZ, percentMillennial, percentBabyBoomer], // Persentase generasi
    ...
    <script>
        // Ambil data dari controller untuk kunjungan
